...
pontos não colineares no espaço 2D.

Podemos realizar o preenchimento do polígono (triângulo) definido entre esses 3 pontos usando a equação:

```

C = αC₀ + βC₁ + γC₂, onde α + β + γ = 1

```

![Preenchimento de triângulos usando coordenadas baricêntricas](images/img7.png)  
...
Para evitar redundância, descreveremos apenas o conteúdo que adicionamos ao arquivo.

### Função `barycentric_coordinate`

Esta função recebe 3 vértices e:

1. Calcula o **bounding box**, ou seja, o retângulo mínimo que envolve o triângulo.
2. Calcula a **área total** do triângulo `abc`.
3. Percorre todos os pixels no bounding box:

   * Calcula as áreas `pbc`, `apc` e `abp`
   * Calcula os coeficientes `α`, `β` e `γ`
   * Se os três forem maiores ou iguais a zero, o ponto está dentro do triângulo → **pinta o pixel**

### Função `render_faces_filled`

Para cada face triangular:

1. Obtém os 3 vértices.
2. Aplica **rotação de 180 graus** nos vértices.
3. Projeta os pontos de 3D para 2D.
4. Chama a função `barycentric_coordinate(...)` para preencher a face.
